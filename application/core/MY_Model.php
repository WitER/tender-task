...
class SimpleModel extends MY_Model
{
    ...
    {
        $next = $this->query("SHOW TABLE STATUS LIKE '".$this->_table."'");
        $next = $next->row(0);
        $next->Auto_increment;
    ...
    public function get_by()
    {
        ...
        $row = $this->_database->get($this->_table, 1)
                        ->{$this->_return_type()}();
        $this->_temporary_return_type = $this->return_type;
    ...
    public function get_all()
    {
        ...
        $result = $this->_database
            ->get($this->_table); //print_r('<pre>'.$this->_database->last_query().'</pre>');
        $result = $result
            ->{$this->_return_type(1)}();
        $this->_temporary_return_type = $this->return_type;
    ...
    {
        $relationship = is_array($relationship) ? $relationship : array($relationship);
        foreach ($relationship as $relate) {
            if (!in_array($relate, $this->_with)) {
                $this->_with[] = $relate;
            }
        }
    ...
    public function relate($row)
    {
        ...
        foreach ($this->belongs_to as $key => $value) {
            ...
            $this->_prepareRelate($row, $relationship, $options, 'get_by');
            unset($relationship, $options);
        }
        ...
        foreach ($this->has_many as $key => $value) {
            ...
            $this->load->model($options['model'], $relationship . '_model');
            $this->_prepareRelate($row, $relationship, $options, 'get_many_by');
            unset($relationship, $options);
        }
    ...
    private function _prepareRelate(&$row, $relationship, $options, $method)
    {
        $where = array();
        if (!empty($options['field'])) {
            $where[$options['field']] = is_object($row) ? $row->{$options['primary_key']} : $row[$options['primary_key']];
            if (!empty($options['secondary_field'])) {
                $where[$options['secondary_field']] = is_object($row) ? $row->{$options['secondary_key']} : $row[$options['secondary_key']];
            }
        } else {
            if ($method == 'get_by') {
                $where['id'] = (int)(is_object($row) ? $row->{$options['primary_key']} : $row[$options['primary_key']]);
            } else {
                $where[$options['primary_key']] = (int)(is_object($row) ? $row->{$this->primary_key} : $row[$this->primary_key]);
            }
        }
        ...
        if (is_object($row)) {
            $row->{$relationship} = $this->{$relationship . '_model'}->{$method}($where);
        } else {
            $row[$relationship] = $this->{$relationship . '_model'}->as_array()->{$method}($where);
        }
        unset($where);
    }
